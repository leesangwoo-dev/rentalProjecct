...
public class EquipmentDAO {
	...
	/**
	 * 특정 상태의 장비 목록을 조회합니다. 이 메서드는 SP_GET_EQUIPMENT_VIEW 프로시저를 호출하며, P_STATE 파라미터에만
	 * 값을 전달하고 나머지 검색 조건은 NULL로 설정합니다.
	 *
		...
		try (Connection conn = getConnection(); CallableStatement cstmt = conn.prepareCall(sql)) {
			...
			try (ResultSet rs = (ResultSet) cstmt.getObject(5)) { // 5번째 파라미터에서 결과 커서 가져오기
				while (rs.next()) {
					...
					// dto.setOfficeGu(rs.getString("OFFICE_GU")); // 프로시저에서 OFFICE_GU를 반환하도록 수정했어야
					// 함

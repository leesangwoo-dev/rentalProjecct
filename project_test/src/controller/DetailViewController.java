...
public class DetailViewController {
    ...
    @FXML private Label eqNameLabel;      // 장비 이름
    @FXML private Label stateLabel;       // 장비 상태
    @FXML private TextArea eqInfoArea;    // 장비 설명 + 취득일 포함

    @FXML private ImageView toolImageView;  // 장비 이미지

    @FXML private Label SerialNumLabel;   // 일련번호
    @FXML private Label costLabel;        // 대여료
    @FXML private Label officeLabel;      // 대여소
    @FXML private Label phoneLabel;       // 전화번호

    @FXML private DatePicker rentDatePicker;    // 대여일
    @FXML private DatePicker returnDatePicker;  // 반납일
    ...
    public void setSerialNumber(String serialNum) {
        EquipmentDetailViewDTO dto = dao.getEachEquipmentDetail(serialNum);
        if (dto == null) return;
        ...
        switch (state) {
            case "사용가능":
                stateLabel.setStyle("-fx-background-color: #B5F3C1; -fx-text-fill: green; -fx-alignment: center; -fx-font-size: 18px; -fx-padding: 2 6; -fx-border-radius: 4; -fx-background-radius: 4;");
                break;
            case "대여":
                stateLabel.setStyle("-fx-background-color: #FFB6B6; -fx-text-fill: red; -fx-alignment: center; -fx-font-size: 18px; -fx-padding: 2 6; -fx-border-radius: 4; -fx-background-radius: 4;");
                break;	
            case "수리":
                stateLabel.setStyle("-fx-background-color: #FFF6B3; -fx-text-fill: goldenrod; -fx-alignment: center; -fx-font-size: 18px; -fx-padding: 2 6; -fx-border-radius: 4; -fx-background-radius: 4;");
                break;
            default:
